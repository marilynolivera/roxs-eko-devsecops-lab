...
router.get('/', function(req, res, next) {
    ...
    if (!check_logged(req, res)) {
        return; // Si se hizo redirect, no continuar
    }
...
router.all('/products/buy', function(req, res, next) {
    ...
    try {
        ...
        cart = {
            ...
            price: params.price.replace(/[$€]/g, '').trim() // remove "$" or "€" symbol
        }
    ...
    db_products.purchase(cart)
        ...
        .catch(function (err) {
            ...
            return res.status(500).json({message: "Error al procesar la compra: " + err.message});
        });
